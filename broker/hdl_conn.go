...
import (
	...
	"github.com/unit-io/unite/types"
)
...
func (c *Conn) onPublish(pkt lp.Publish, messageID uint16, msgTopic []byte, payload []byte) *types.Error {
	...
	// Check whether the key is 'unite' which means it's an API request
	if len(topic.Key) == 5 && string(topic.Key) == "unite" {
		c.onuniteRequest(topic, payload)
		return nil
	}
...
func (c *Conn) onuniteRequest(topic *security.Topic, payload []byte) (ok bool) {
	var resp interface{}
	defer func() {
		if b, err := json.Marshal(resp); err == nil {
			c.SendMessage(&message.Message{
				Topic:   []byte("unite/" + string(topic.Topic[:topic.Size])),
				Payload: b,
			})
		}
	}()
