...
func (a *adapter) Open(jsonconfig string, reset bool) error {
	...
	a.mem, err = memdb.Open(opts, memdb.WithLogFilePath(config.Dir), memdb.WithLogSize(config.Size), memdb.WithBufferSize(config.Size))
	if err != nil {
		return err
	}
...
func (a *adapter) PutMessage(key uint64, payload []byte) error {
	if err := a.mem.Set(key, payload); err != nil {
		return err
	}
	return nil
}
